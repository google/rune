...
## Stage 3 runtime layout: compact aligned headed values

Rune arrays now use a 32-byte x86-64 header: element size, capacity, used
length, and one full-width diagnostic sentinel immediately before the payload.
This saves 8 requested bytes per allocation versus the former 40-byte header
while moving every payload from 8-mod-16 to 0-mod-16 alignment. The removed
leading sentinel was redundant diagnostics, not a security boundary; checked
size arithmetic, header validation, the adjacent sentinel, and sanitizer
coverage remain.

The final same-source fannkuch-redux controls were generated by compiler
SHA-256 `92739eb586362e07b4a58bc6c508a94df8e930b084c3057184a12f960734b50a`
from benchmark source SHA-256
`a4fe64dc977112a1b2974338c41775a9f0aa5f83fcdacc55f60eb037bc77bc21`.
H32 is the production output. H40 and H48 differ from that generated C only by
one or two named 64-bit padding words and matching layout assertions; all
checked allocation code and benchmark code are identical. All three reproduce
the committed N=10 golden byte-for-byte and cross-match at timing N=12.

| Fannkuch layout comparison | H32 wins | H32/other best | H32/other mean | H32/other median |
|---|---:|---:|---:|---:|
| H32 versus former-size H40 | 10/10 | 0.988390x | 0.991339x | 0.990018x |
| H32 versus aligned H48 | 4/10 | 0.999539x | 0.999826x | 1.001038x |

Each row is an independent quiet CPU-0 series: powersave governor, one warmup
per member, ten alternating pairs, low process priority, and fail-closed
logical-CPU checks at every boundary. The H40 TSV SHA-256 is
`f184fd79667668b0971a367dcd4a44a010cfdbef5a946fd8ceeacbff855e0cb1`;
the H48 TSV SHA-256 is
`bfe382c7db3daca498511709705ff59a18ab00e677fb215174320f8064415f10`.
Thirty-nine all-mode attempts and two H48-only attempts were discarded before
publication for external load or CPU contention; no threshold was relaxed.

H32 therefore dominates H40 and is measurement-parity with equally aligned
H48. There is no useful RAM/speed choice to expose as a flag: the production
layout is both the smallest and the fastest supported option in this control.

The same checkpoint also makes every nonnull Rune-visible string headed,
deduplicates static headed literals, preserves embedded NUL bytes in string
operations and `[u8]` conversions, and fails closed on array size overflow or
reallocation failure. The final isolated bootstrap gate is `PASS=209 FAIL=0`;
...
## Stage 3 binary-trees migration: lexical regions

Binary-trees now uses ordinary nullable class fields instead of ownership
relations and places each short-lived tree in a validated `withRegion` scope.
The stretch tree also uses a region. The long-lived tree is deliberately built
before the short-tree loops on the ordinary class-pool path and checked after
them, preserving the benchmark's required lifetime. Every node is still
allocated and visited; generated-C and source audits found no closed-form
checksum, skipped construction, or benchmark recognition in the compiler.

The frozen candidate was generated by compiler SHA-256
`e030311d674bab59c980f8a64ce913f02d4a4e2c5dd1f9ab3e252d987ad3e1b4`
from source SHA-256
`73e20e96c8309cdd4f376458a6d272001e12a1381c91aa666df3384c2394ad43`.
Checked and unsafe Clang O3, unsafe Clang O0, and GCC O3 all match the 223-byte
committed N=10 golden and the 455-byte N=21 leader output. The Clang
ASan/UBSan run matches at N=10. The generated region node is 24 bytes with an
exact 24-byte stride: two 32-bit Rune header words followed by two pointers.

Each row below is an independent, fail-closed CPU-0 series with one warmup per
member and ten alternating pairs. Load had to be at most 1.0 before a series;
before every member CPU 0 had to be at most 10% busy and every other logical
CPU at most 50% busy. An interrupted series installed no TSV. Times are median
wall seconds; ratios are paired-series aggregate ratios, not cross-session
substitutions.

| Binary-trees N=21 comparison | Left median | Right median | Left/right median | Result |
|---|---:|---:|---:|---|
| old unsafe Rune / region unsafe Rune | 5.536 | 2.001 | 2.767x | region is **2.77x faster** |
...
Gprof attributes 68.75% of sampled CPU time to `makeTree` and 29.51% to
`Node.check`. Rune enters `makeTree` once for each of 613,766,494 nodes; its
hot path repeats the TLS region lookup, capacity/alignment checks, header and
zero initialization, and carries the inactive ordinary-pool branch. The
profile SHA-256 is
`a03a776e0395087a11f2f3bc988f1914b28d45024229ee46eca6d127ef7e28e8`.

Read-only generated-C experiments on low-priority CPU 15 are attribution, not
published timings. They show that GCC's recursive peeling is roughly 29.5%
faster than Clang on unchanged C; under GCC, a distinct 16-byte headerless
region representation is about 14.4% faster, and using the leader's valid
both-children-or-neither traversal invariant adds about 2.9%. Removing
redundant zeroing is about 2.8%. Conversely, raising the per-thread cache cap
